...
The job changes into the code folder (where uv finds the project), runs `signals run` (sync,
record the week's leads, write the digests) and then `signals purge` (retention), logging to
~/Signals/logs/weekly.log.
...
def launchd_plist(project: Path, uv: str, day: str, hour: int, minute: int, log: Path) -> bytes:
...
def cron_line(project: Path, uv: str, day: str, hour: int, minute: int, log: Path) -> str:
    return f"{minute} {hour} * * {DAYS.index(day)} ({job_command(project, uv)}) >> {shlex.quote(str(log))} 2>&1"
